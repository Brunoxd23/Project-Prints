    ...
    },
    {
      ...
      action: async (page) => {
        try {
          ...
          // Para Sustentabilidade e Inovação, esperamos 16 accordions
              ...
              if (i < totalSlides - 1) {
                ...
                const navigationSuccess = await page.evaluate(
                  (targetSlideIndex) => {
                    ...
                    console.log(
                      `❌ Nenhum método de navegação funcionou para slide ${
                        targetSlideIndex + 1
                      }`
                    );
                    return false;
                ...
                if (navigationSuccess) {
                  ...
                  if (
                    currentSlide &&
                    currentSlide.includes(`${i + 2} of ${totalSlides}`)
                  ) {
                    console.log(`✅ Navegação confirmada para slide ${i + 2}!`);
                  } else {
                    console.log(
                      `⚠️ Navegação pode não ter funcionado corretamente`
                    );
                  }
                } else {
...
router.post("/run-script-sustentabilidade-quinzenal", async (req, res) => {
  try {
    ...
    return res.json(files); // Retorna os caminhos completos
  } catch (error) {
    return res.status(500).json({ error: error.message }); // Retorna erro caso ocorra
  }
});
