          ...
          if (totalSlides > 1) {
            ...
            for (let i = 0; i < totalSlides; i++) {
              ...
              const navigationSuccess = await page.evaluate(
                (targetSlideIndex) => {
                  console.log(
                    `🎯 Tentando navegar para slide ${targetSlideIndex}...`
                  );

                  // Método mais direto: clicar diretamente no dot pelo ID específico
                  const dotId = `slick-slide-control${
                    10 + targetSlideIndex - 1
                  }`;
                  ...
                  if (targetSlideIndex === 1) {
                    ...
                    if (
                      prevButton &&
                      !prevButton.classList.contains("slick-disabled")
                    ) {
              ...
              if (navigationSuccess) {
                ...
                console.log(
                  `📍 Slide atual após navegação: ${currentSlideInfo}`
                );
                ...
                if (
                  currentSlideInfo &&
                  currentSlideInfo.includes(expectedSlide)
                ) {
                  console.log(`✅ Confirmação: estamos no slide ${i + 1}!`);
                } else {
                  console.log(
                    `⚠️ Aviso: pode não estar no slide correto. Esperado: ${expectedSlide}, Atual: ${currentSlideInfo}`
                  );
                }
              } else {
    ...
    },
    {
      ...
      action: async (page) => {
        try {
          ...
          await page.waitForSelector("button.accordion.template.campo", {
            visible: true,
            timeout: 10000,
          });
          ...
          const faqAccordions = await page.$$(
            "button.accordion.template.campo"
          );
          ...
          console.log(
            `🎠 Accordions FAQ detectados: ${totalAccordions} (fixo para Infraestrutura/Facilities)`
          );
...
router.post("/run-script-infraestrutura-mensal", async (req, res) => {
  try {
    ...
    const files = screenshotFiles.map(
      (filename) =>
        `/${courseInfo.courseFolder}/${courseInfo.subcourseFolder} ${semesterFolder}/${filename}`
    );
