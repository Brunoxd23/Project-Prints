    ...
    },
    {
      ...
      action: async (page) => {
        try {
          ...
          const totalSlides = 4;
          console.log(
            `🎠 Carrossel detectado com ${totalSlides} slides (fixo para Bases da Saúde)`
          );
    ...
    },
    {
      ...
      action: async (page) => {
        try {
          ...
          console.log(
            `🎠 Accordions FAQ detectados: ${totalAccordions} (fixo para Dependência Química)`
          );
...
router.post("/run-script-bases-integrativa-mensal", async (req, res) => {
  try {
    const url =
    ...
    if (req.body && req.body.semester) {
      ...
      if (!semesterRegex.test(customSemester)) {
        ...
        return res.status(400).json({
          error:
            "Semestre inválido. Use o formato YYYY-N (ex: 2025-1, 2025-81...)",
        });
      }
    ...
    const files = screenshotFiles.map(
      (filename) => `/BSI_Mensal_${semesterFolder}/${filename}`
    );
