...
router.post("/run-script-bases-integrativa-mensal", async (req, res) => {
  try {
    ...
    if (req.body && req.body.semester) {
      ...
      if (!semesterRegex.test(customSemester)) {
        ...
        return res.status(400).json({ 
          error: "Semestre inválido. Use o formato YYYY-N (ex: 2025-1, 2025-81...)" 
        });
      }
    ...
    const courseInfo = getCourseFolderName("Bases da Saúde Integrativa e Bem-Estar", "Unidade Paulista | Mensal");
